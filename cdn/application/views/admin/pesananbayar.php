	...
	<table class="table table-condensed table-hover">
		...
		?>
			<tr>
				...
				<td><?=$this->security->xss_clean($this->func->getProfil($r->usrid,"nama","usrid"))?> &nbsp;<?=$lepel?></td>
				<td><?=$this->func->formUang($r->total-$r->kodebayar+$r->biaya_cod)?></td>
				<td><?=$this->func->formUang($r->kodebayar)?></td>
				<td><?=$kurir?></td>
				<td style="min-width:220px">
					<?php if($r->ipaymu == "" OR $bukti != ""){?><a href="javascript:konfirm(<?=$r->id?>)" class="btn btn-success btn-xs m-b-4"><i class="fas fa-check"></i> Verifikasi</a><?php } ?>
					<a href="javascript:detail(<?=$trxid?>)" class="btn btn-primary btn-xs m-b-4"><i class="fas fa-list"></i> Detail</a>
