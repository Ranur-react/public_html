	...
	<table class="table table-condensed table-hover">
		...
		?>
			<tr>
				...
				<td><?=$this->security->xss_clean($this->func->getProfil($r->usrid,"nama","usrid"))?></td>
				<td><?=$kurir?><br/><?=$this->func->formUang($r->ongkir)?></td>
				<td>
					<a href="javascript:cetak(<?=$r->id?>)" class="btn btn-warning btn-xs m-b-4"><i class="fas fa-print"></i> Invoice</a>
					<a href="javascript:detail(<?=$r->id?>)" class="btn btn-primary btn-xs m-b-4"><i class="fas fa-list"></i> Detail</a>
					<?php if($r->kurir == "cod" OR $r->kurir == "toko"){ ?>
					<a href="javascript:void(0)" onclick="kirimPaket(<?=$r->id?>)" class="btn btn-success btn-xs m-b-4"><i class="fas fa-shipping-fast"></i> Kirim Pesanan</a>
					<?php }else{ ?>
					<a href="javascript:void(0)" onclick="inputResi(<?=$r->id?>)" class="btn btn-success btn-xs m-b-4"><i class="fas fa-shipping-fast"></i> Resi</a>
					<?php } ?>
