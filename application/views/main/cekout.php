					...
					<div class="section p-all-30">
						...
						<?php
							if($data->transfer > 0){
								...
								$biayacod = $set->biaya_cod > 100 ? $set->biaya_cod : $bayartotal*($set->biaya_cod/100);
						?>
							<div class="p-b-43">
								<div class="row">
									<div class="col-md-12 m-b-20">
										<h5 class="text-black">Mohon lakukan pembayaran sejumlah</h5><br/>
								...
								<div class="row p-t-5 m-t-30 bayarcod" style="display:none;">
									<div class="col-md-12 m-b-20">
...
<script type="text/javascript">
	...
	function bayarOtomatis(){
		...
		$.post("<?=site_url('assync/updatepesanan')?>",{"id":"<?=$data->id?>","metode":3,[$("#names").val()]:$("#tokens").val()},function(ev){
			...
			if(data.success == true){
				...
				swal.fire("Gagal bayar Ipaymu","Pembayaran melalui IPAYMU sedang terkendala, silahkan hubungi admin toko untuk memperbaiki kendala ini.","danger");
			}
		});
	}
	...
	<?php
		if(!isset($_GET["ubahmetode"])){
			$metode = $data->metode == 0 ? $data->metode : 2;
			if($metode == 1){
				echo "bayarCOD();";
			}elseif($metode == 2){
				echo "bayarManual();";
