    ...
    <table class="table table-hover">
      ...
      <?php
        foreach($saldo->result() as $res){
          ...
          $status = ($res->darike == 3) ? $st->status : 1;
          $status = ($status == 1) ? "<span class='text-success'><i class='fas fa-check-circle'></i> Berhasil</span><br/><small>".$this->func->ubahTgl("d M Y H:i",$st->selesai)."WIB</small>" : "<span class='text-warning'><i class='fas fa-clock'></i> Sedang Diproses</span>";
          $status = ($status == 2) ? "<span class='text-success'><i class='fas fa-times-circle'></i> Dibatalkan</span><br/><small>".$this->func->ubahTgl("d M Y H:i",$st->selesai)."WIB</small>" : $status;
